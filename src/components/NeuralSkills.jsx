...
const SkillCard = ({ skill, idx }) => {
  ...
  return (
    <motion.div
      ...
      className="relative group bg-glass-bg backdrop-blur-md border border-glass-border rounded-xl p-4 hover:border-neon-cyan transition-colors cursor-pointer z-10 hover:z-50"
    ...
    >
      ...
      <div 
        className={`absolute ${classes.modalPosition} w-48 md:w-56 p-4 bg-[#050505]/95 backdrop-blur-xl border border-white/10 group-hover:border-neon-cyan/50 rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible scale-75 group-hover:scale-100 transition-all duration-300 pointer-events-none shadow-2xl shadow-black/80 flex flex-col gap-1.5 z-50`}
        style={{ marginTop, marginLeft }}
      >
        {/* Angled Connecting Thin Line */}
        <div 
          className={`absolute ${classes.linePosition} w-6 h-[1px] bg-neon-cyan/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100`}
        ></div>
        
        <h4 className="text-neon-cyan font-mono font-bold text-sm border-b border-white/10 pb-1 mb-1">
          {skill.title}
        </h4>

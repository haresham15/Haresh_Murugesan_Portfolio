...
const NowSection = () => {
  ...
  return (
    <section id="now" className="w-full max-w-4xl mx-auto px-4 py-20">
      <h2 className="text-4xl md:text-6xl font-mono font-bold tracking-tight text-light-gray mb-12 flex items-center gap-4">
      ...
      <div className="space-y-8 pl-4 md:pl-8 border-l-2 border-white/10">
        {posts.map((post, index) => (
          ...
          >
            {/* Timeline dot */}
            <div className="absolute -left-[21px] md:-left-[37px] top-2 h-3 w-3 rounded-full bg-neon-cyan/50 border border-neon-cyan"></div>
            
            <span className="text-neon-cyan font-mono text-sm block mb-2">{post.date}</span>
            <h3 className="text-xl font-mono font-bold text-white mb-2">{post.title}</h3>
